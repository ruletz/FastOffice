...
//            RELEASEINTERFACE(event);
        ...
        if ( cmd.find(L"app:onready") != std::wstring::npos ) {
            ...
            // OPTIMIZED: Always hide cloud connect panel
...
//                in_new_window = true;
        ...
        if ( arg.rfind(app_action, 0) == 0 ) {
            ...
            if ( argScheme.rfind(app_action_plugin, 0) == 0 ) {
                // Plugins are disabled in minimal builds.
                // Ignore "install-plugin" deeplinks to keep attack surface and bundle size down.
            } else {
...
//    QApplication::setStyle(new CStyleTweaks);
    ...
    // Minimal-build feature gates for embedded web-apps/sdkjs.
    // Default upstream behavior is preserved unless explicitly set here.
    // This is consumed in `web-apps/apps/common/main/lib/util/desktopinit.js` and then by sdkjs.
    EditorJSVariables::setVariable("editorFeatures", QJsonObject{
        {"collaboration", false},
        {"plugins", false},
        {"ai", false},
        {"cloud", false},
        {"showHeaderFooterEditMarks", false},
        {"uiMinimal", true}
    });
