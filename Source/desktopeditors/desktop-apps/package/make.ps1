...
Write-Host "DELETE: $BuildDir\\desktop\\editors\\sdkjs-plugins (minimal build)"
Remove-Item -Force -Recurse -ErrorAction SilentlyContinue -LiteralPath "$BuildDir\\desktop\\editors\\sdkjs-plugins"

Write-Host "PRUNE: web-apps locales (keep en + pt-PT)"
$keepEditorLocales = @("en.json", "pt-pt.json")
Get-ChildItem -File -Recurse `
    -Path "$BuildDir\\desktop\\editors\\web-apps\\apps" `
    -Filter "*.json" `
    | Where-Object { $_.DirectoryName -match "\\\\locale$" } `
    | ForEach-Object {
        if ($keepEditorLocales -notcontains $_.Name.ToLowerInvariant()) {
