...
$(document).ready(function() {
    ...
    const editorFeatures = nativevars.editorFeatures || {};
    const cloudEnabled = false; // OPTIMIZED: Cloud option removed

    const _toolmenu_tpl = `
            <div class="main-column col-left tool-menu icons-only">
              <li class="menu-item">
                <a action="recents">
                    <div class="icon-box">
    ...
    if (cloudEnabled && !!window.ControllerPortals)
    ...
    if (!cloudEnabled || !window.config.portals.checklist) {
        $('.tools-connect').hide();
        hideAction('connect');
        if (!cloudEnabled)
            console.log('Cloud features disabled');
        else
