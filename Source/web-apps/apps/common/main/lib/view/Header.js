    ...
    Common.Views.Header =  Backbone.View.extend(_.extend(function(){
        ...
        var templateRightBox = '<section>' +
                            ...
                            '<section id="box-right-btn-group" style="display: inherit;">' +
                                '<div class="hedset">' +
                                    '<div class="btn-slot" id="slot-hbtn-undo"></div>' +
                                    '<div class="btn-slot" id="slot-hbtn-redo"></div>' +
                                    '<div class="btn-slot" id="slot-hbtn-save"></div>' +
                                '</div>' +
                                '<div class="hedset">' +
        ...
        return {
            ...
            getPanel: function (role, config) {
                ...
                if ( role == 'right' ) {
                    ...
                    if ( !config.twoLevelHeader) {
                        // OPTIMIZED: Save, Undo, Redo moved from title bar to tab bar right area
                        if (config.showSaveButton || config.isEdit) {
                            var save_icon = config.canSaveToFile || config.isDesktopApp && config.isOffline ? 'btn-save' : 'btn-download';
                            me.btnSave = createTitleButton('toolbar__icon icon--inverse ' + save_icon, $html.findById('#slot-hbtn-save'), true, 'bottom', 'big', 'S');
                            if (appConfig && (appConfig.canSaveToFile || appConfig.isDesktopApp && appConfig.isOffline)) {
                                me.shortcutHints.Save = { btn: me.btnSave, label: me.tipSave };
                            } else {
                                me.btnSave.updateHint(me.tipDownload);
                            }
                        }
                        me.btnUndo = createTitleButton('toolbar__icon icon--inverse btn-undo icon-rtl', $html.findById('#slot-hbtn-undo'), true, 'bottom', 'big', 'Z',
                                                        [Common.enumLock.undoLock, Common.enumLock.fileMenuOpened, Common.enumLock.lostConnect]);
                        me.shortcutHints.EditUndo = { btn: me.btnUndo, label: me.tipUndo };
                        me.btnRedo = createTitleButton('toolbar__icon icon--inverse btn-redo icon-rtl', $html.findById('#slot-hbtn-redo'), true, 'bottom', 'big', 'Y',
                                                        [Common.enumLock.redoLock, Common.enumLock.fileMenuOpened, Common.enumLock.lostConnect]);
                        me.shortcutHints.EditRedo = { btn: me.btnRedo, label: me.tipRedo };
