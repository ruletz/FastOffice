...
(function (window, undefined) {
	...
	CGraphics.prototype.DrawHeaderEdit = function (yPos, lock_type, sectionNum, bIsRepeat, type) {
		// Allow host app to disable header/footer edit markers entirely (minimal UI builds)
		if (AscCommon && AscCommon.EditorFeatures && AscCommon.EditorFeatures.showHeaderFooterEditMarks === false)
			return;
	...
	CGraphics.prototype.DrawLockParagraph = function (lock_type, x, y1, y2) {
		// Hard-disable collaborative visuals when collaboration is disabled by host app
		if (AscCommon && AscCommon.EditorFeatures && AscCommon.EditorFeatures.collaboration === false)
			return;
	...
	CGraphics.prototype.DrawSpellingLine = function (y0, x0, x1, w) {
		if (!Asc.editor.isViewMode) {
			...
			var _m = this.m_oFullTransform;
			var _x0 = (_m.TransformPointX(x0, y0) >> 0) + 0.5;
			var _x1 = (_m.TransformPointX(x1, y0) >> 0) + 0.5;
			var _y = (_m.TransformPointY(x0, y0) >> 0) + 0.5 + 1.0;
			...
			if (waves > 0) {
				...
				for (var i = 0; i < waves; i++) {
					var cp1x = _x0 + (i * wl + wl / 4);
					var cp1y = _y + amplitude;
					var cp2x = _x0 + (i * wl + wl * 3 / 4);
					var cp2y = _y - amplitude;
					var ep_x = _x0 + (i * wl + wl);
					var ep_y = _y;
