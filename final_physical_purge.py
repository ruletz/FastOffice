...
def physical_strip(path, is_hub=False):
    if not os.path.exists(path): return
    with codecs.open(path, 'r', 'utf8') as f:
        text = f.read()

    # --- 1. THE PERMANENT PURGE ---
    # Targets for physical deletion from the DOM string
    tags_to_strip = [
    ...
    # --- 2. PRECISION CSS (NO FONT CHANGE, NO BG CHANGE) ---
    precision_css = """
<style id="nuclear-final-logic-v24">
/* UNIFY HEADER */
...
/* CENTER TITLE ONLY (Preserve Font) */
...
/* FIX HOMESCREEN ICONS (Transparent Background) */
...
# Process files
physical_strip(hub_path, is_hub=True)
for editor in editors:
    physical_strip(f"c:/Projects/OnlyOffice/app/editors/web-apps/apps/{editor}/main/index.html", is_hub=False)
    # Loader
...
print("DEFINITIVE PHYSICAL PURGE COMPLETE: Languages, Cloud, Plugins, and Bloat deleted.")
