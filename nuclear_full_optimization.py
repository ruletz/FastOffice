...
/* 2. UNIFY HEADER: Pin Toolbar to Top */
...
/* 4. CLUTTER REMOVAL (Tabs & Sidebar) */
...
/* 5. HOMESCREEN: COLORFUL BIG ICONS */
...
(function() {
    ...
    async function BootRust() {
        try {
            ...
            console.log('[RUST] OOXML Engine Optimized');
        } catch(e) { console.error('Rust Init Failed', e); }
    }
    BootRust();

    // 2. PERSISTENT HOMESCREEN GRID
    function injectGrid() {
        var placeholder = document.getElementById('placeholder');
        if (placeholder && !document.querySelector('.document-creation-grid')) {
            var grid = document.createElement('div');
            grid.className = 'document-creation-grid';
            grid.innerHTML = `
    ...
    var obs = new MutationObserver(function() {
        if (document.querySelector('.col-center') && !document.querySelector('.document-creation-grid')) injectGrid();
    });
...
# Apply All-in-One to Hub and Editors
def deploy_optimization(path):
    if not os.path.exists(path): return
    with codecs.open(path, 'r', 'utf8') as f:
        text = f.read()
    
    # Surgical injection: Remove previous and add new
...
deploy_optimization(hub_path)
for editor in editors:
    deploy_optimization(f"c:/Projects/OnlyOffice/app/editors/web-apps/apps/{editor}/main/index.html")
...
print("NUCLEAR FULL OPTIMIZATION DEPLOYED: Every win re-applied.")
