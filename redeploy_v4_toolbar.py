...
# --- HELPERS ---
def inject_head(text, style="", script="", id_suffix="toolbar"):
    if '</head>' in text:
        injection = ""
        if style: injection += f'\n<style id="v4-style-{id_suffix}">{style}</style>'
        if script: injection += f'\n<script id="v4-script-{id_suffix}">{script}</script>'
...
def apply_editor_ui():
    print("[3/5] Applying Editor UI changes (Single Toolbar Mode)...")
...
/* Move Header Icons (Save, Undo, Redo) to the right of the single bar */
#box-header-tools {
...
/* Center Document Title in the single bar */
#box-document-title {
...
/* Clean up Toolbar Tabs (Home, Insert, etc) */
...
"""
        text = inject_head(text, style, script, "toolbar")
...
if __name__ == "__main__":
    ...
    apply_hub_ui()
    apply_editor_ui()
    print("\\nSUCCESS: Single Toolbar Mode deployed.")
