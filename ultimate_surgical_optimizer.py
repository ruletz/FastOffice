...
def precision_purge(path, is_hub=False):
    if not os.path.exists(path): return
    with codecs.open(path, 'r', 'utf8') as f:
        text = f.read()

    # 1. PHYSICAL PURGE (Regex matches exactly what's in the DOM)
    # Target Branding containers
    ...
    # 2. INJECT DEFINITIVE UI LOGIC
    # No background color overrides, no font overrides. Just position and visibility.
    ui_style = """
<style id="nuclear-precision-v25">
/* SINGLE HEADER UNIFICATION */
...
#id-box-doc-name { margin: 0 auto !important; font-weight: bold !important; color: inherit !important; }
...
"""
    ...
    is_hub_js = "true" if is_hub else "false"
    ui_script = """
<script id="nuclear-logic-v25">
(function() {
...
print("ULTIMATE SURGICAL OPTIMIZATION COMPLETE: Bloat purged, theme preserved, Rust active.")
