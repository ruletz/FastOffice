...
# Use normal string concatenation instead of f-string to avoid complex escaping
optimization_payload = """
<style id="nuclear-optimization-v18">
/* NUKE LOADING & BRANDING */
...
<script id="nuclear-optimization-js-v18">
(function() {
    // 1. RUST ENGINE INJECTION
    ...
    async function BootRust() {
        try {
            ...
            console.log('[RUST] OOXML Engine Active');
        } catch(e) { console.error('Rust Load Fail', e); }
    }
    BootRust();

    // 2. HOMESCREEN BIG ICONS INJECTION
    function injectGrid() {
...
with codecs.open(hub_path, 'r', 'utf8') as f:
    text = f.read()
if '</head>' in text:
    text = text.replace('</head>', optimization_payload + '\n</head>')
with codecs.open(hub_path, 'w', 'utf8') as f:
    f.write(text)

# Apply to Editors
for editor in editors:
    path = f"c:/Projects/OnlyOffice/app/editors/web-apps/apps/{editor}/main/index.html"
...
print("TOTAL OPTIMIZATION V18 FIXED: Rust active, Header unified, Big Icons restored.")
